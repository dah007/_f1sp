...
const CardContainer: React.FC<CardProps> = ({
    ...
    return (
        <Card className={cn('w-full bg-white dark:bg-slate-800 shadow-md p-0', className)}>
            {(title || description) && (
                <CardHeader>
                    {title && <CardTitle className={cn('p-4 pt-4 pb-0 m-0', titleClassName)}>{title}</CardTitle>}
